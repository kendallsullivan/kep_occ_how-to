...
def calcBinomialFPEffectiveness(model = "rotatedLogisticX0", path = os.getcwd(), stellarType = 'GK', rpmin = 0.5, rpmax = 15, pmin = 1, pmax = 400, mesmin = 7, mesmax = 30, verbose = True, plots = True, nwalkers = 100, nsteps = 1e4, nfits = 100):
	...
	spIndex = np.where(np.all([
		...
		syntheticTrueFps['period']>pmin,syntheticTrueFps['period']<pmax], axis=0))
	...
	# nwalkers, run length and # of burnin steps must be the same in binomialObsFPRate and binomialFPEffectivess
	...
	#impose 500 burnin steps - probably worth testing at some point
...
def main(argv):
	...
	try:
		...
		short_options = 'm:i:v:s:t:p:P:r:R:n:N:f:' #model name, plots y/n, verbose y/n, savepath for files, spectral types to include, period min, period max, rp min, rp max, nwalkers, nsteps
		long_options = 'model:images:verbose:savepath:spt:pmin:pmax:rpmin:rpmax:nwalk:nstep:nfit:' 
