...
def calcBinomialFPRate(model = 'rotatedLogisticX0', path = os.getcwd(), stellarType = 'GK', rpmin = 0.5, rpmax = 15, pmin = 1, pmax = 400, mesmin = 7, mesmax = 30, verbose = True, plots = True, nwalkers = 100, nsteps = 1e4, nfits = 100)
    ...
    if lnBinprior(maxLikelihoodResult, tceData, obsModel) == -np.inf:
        ...
        print("Best fit violates prior, replacing maxLikelihoodResult result with initialPos")
    ...
    # nwalkers, run length and # of burnin must be the same in binomialObsFPRate and binomialFPEffectivess
